var App = angular.module("won_lead");

App.controller("WonLeadCtrl", [
  "$scope",
  "$rootScope",
  "$state",
  "$route",
  ...
  function (
    ...
    $scope.add_sites_activated = function(id) {

      $scope.lead_id = id;
      // angular.element("#add_site_activated").modal("show");

    };

    $scope.already_activated = function() {
      // SweetAlert.swal("Error!", 'You have aready submitted this site.', "info");
      toastr.info(null, 'You have aready submitted this site.');
    };

    $scope.copyAddress = function(type) {

      if(type == 'billing') {
        $scope.siteActivatedData.billing_to_address = $scope.siteActivatedData.address;
      }

      if(type == 'consignee') {
        $scope.siteActivatedData.consignee_address = $scope.siteActivatedData.address;
      }

      if(type == 'invoice') {
        $scope.siteActivatedData.invoice_address = $scope.siteActivatedData.address;
      }
    ...
    $scope.save_site_activated = function() {

      console.log($scope.siteActivatedData);

      var cost_schedule_file = '';

      if($scope.siteActivatedData.cost_schedule == "undefined" || $scope.siteActivatedData.cost_schedule == undefined || $scope.siteActivatedData.cost_schedule == '' || $scope.siteActivatedData.cost_schedule == null) {
      ...
      if($scope.siteActivatedData.work_order == "undefined" || $scope.siteActivatedData.work_order == undefined || $scope.siteActivatedData.work_order == '' || $scope.siteActivatedData.work_order == null) {

        work_order_file = angular.element("#work_order")[0].files[0];
        if(work_order_file == "" || work_order_file == undefined || work_order_file == null) {
          work_order_file = '';
          // SweetAlert.swal("Error!", 'Please attach your work order', "error");
          // return;
        }

      }

      var data = $scope.siteActivatedData;
      data.cost_schedule = cost_schedule_file;
      data.work_order = work_order_file;
      ...
      objXhr.onreadystatechange = function() {
          ...
          if (objXhr.readyState == 4 && (objXhr.status == 200 || objXhr.status == 201)) {
              ...
              // $route.reload();
              // angular.element("#add_site_activated .close").trigger("click");
              $scope.leadData = {};

              angular.forEach($scope.leadData,function(value,key){
                $scope.leadData[key] = "";
              });

              $scope.searchLeads(1);
              angular.element("#cost_schedule").val("");
              angular.element("#work_order").val("");

          }

      };

      objXhr.open("POST", configSettings.apiUrl + 'site_activated/add', true);
      objXhr.setRequestHeader('Authorization', 'Bearer ' + localStorage.getItem("satellizer_token"))
    ...
    function getLeads(page) {
      ...
      if (
        $scope.date_type_filter != undefined &&
        $scope.date_type_filter != null &&
        $scope.date_type_filter != ""
      ) {
        $scope.search_data.date_type_filter = $scope.date_type_filter;
      }
    ...
    $scope.edit_lost_lead = function (id) {
      ...
      LeadFactory.getLeadDetail(id).then(
        function (response) {
          ...
          $scope.leadData.id = data.id;
          $scope.leadData.company_name = data.company_name;
          // angular.element("#add_lost_lead").modal("show");
        },
        function (error) {
          SweetAlert.swal("Error!", error.message, "error");
        }
