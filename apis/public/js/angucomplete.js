...
angular.module('angucomplete', [] )
    ...
    return {
        ...
        template: '<div class="angucomplete-holder"><input id="{{id}}_value" name="searchStr" ng-model="searchStr" type="text" placeholder="{{placeholder}}" class="{{inputClass}}" ng-keyup="keyPressed($event)" required /><div id="{{id}}_dropdown" class="angucomplete-dropdown" ng-if="showDropdown"><div class="angucomplete-searching" ng-show="searching">Searching...</div><div class="angucomplete-searching" ng-show="!searching && (!results || results.length == 0)">No results found</div><div class="angucomplete-row" ng-repeat="result in results" ng-click="selectResult(result)" ng-mouseover="hoverRow()" ng-class="{\'angucomplete-selected-row\': $index == currentIndex}"><div ng-if="result.image && result.image != \'\'" class="angucomplete-image-holder"><img ng-src="{{result.image}}" class="angucomplete-image"/></div><div>{{result.title}}</div><div ng-if="result.description && result.description != \'\'" class="angucomplete-description">{{result.description}}</div></div></div></div>',
            ...
            $scope.searchTimerComplete = function(str) {
                ...
                if (str.length >= $scope.minLength) {
                    if ($scope.localData) {
                        ...
                        $http.get($scope.url + str, {}).then(function(responseData){
                            $scope.searching = false;
                            $scope.processResults(responseData.data.results);
                        }, function(data){
                            console.log("error");
                        });
